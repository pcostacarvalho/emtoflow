...
The `emtoflow.modules.dos` module provides parsing and plotting functionality
for Density of States (DOS) data from EMTO output files.
...
```python
from emtoflow.modules.dos import plot_dos
...
Used by `emtoflow.modules.optimization_workflow` (via its analysis helpers) for
automated DOS analysis in optimization workflows whenever DOS generation is
enabled in the config, e.g.:

```yaml
generate_dos: true
dos_plot_range: [-0.8, 0.15]
dos_ylim: null           # or [0, 10] to fix the y-axis
```
