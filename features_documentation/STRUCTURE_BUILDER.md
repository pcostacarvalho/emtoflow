...
The `emtoflow.modules.structure_builder` module provides a unified interface for
creating EMTO-compatible structure dictionaries from either CIF files or
user-specified lattice parameters. This enables support for both experimental
structures and custom alloys/ordered compounds.
...
- `emtoflow.modules.lat_detector`: lattice type detection and primitive vectors
- `emtoflow.modules.element_database`: default magnetic moments and properties
- `emtoflow.modules.create_input`: uses structure dictionaries to generate EMTO input files
