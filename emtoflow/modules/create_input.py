...
from emtoflow.modules.structure_builder import (
    create_emto_structure,
    lattice_param_to_sws,
)
from emtoflow.modules.dmax_optimizer import run_dmax_optimization
...
def create_emto_inputs(config):
    ...
    # 'substitutions' is optional in many configs, so use .get() to avoid KeyError
    substitutions = cfg.get('substitutions')
    
    # Import job script functions based on job_system
    if job_system == 'pelle':
        from emtoflow.modules.inputs.jobs_pelle import write_serial_sbatch, write_parallel_sbatch
    else:  # default to tetralith
        from emtoflow.modules.inputs.jobs_tetralith import write_serial_sbatch, write_parallel_sbatch
    ...
    if optimize_dmax:
        ...
        dmax_per_ratio = run_dmax_optimization(
